...
const Classes = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-gray-50 relative pt-20">
      <div className="relative max-w-5xl mx-auto px-6 lg:px-8 py-16 z-10">
        ...
        <div className="text-center mb-20 animate-slide-up">
          ...
          <p className="text-xl text-gray-600 font-light max-w-3xl mx-auto leading-relaxed">
            Descubre tu potencial artístico a través de nuestras clases especializadas en danza y artes escénicas
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {studio.whatsapp && (
                    <div className="flex items-center gap-3 text-gray-600">
                      ...
                      <span className="font-light">WhatsApp: {studio.whatsapp}</span>
                    </div>
                  )}
              ...
              <div className="space-y-8">
                {studio.classes.map((classItem, classIndex) => (
                  <div
                    ...
                    style={{ animationDelay: `${(studioIndex * studio.classes.length + classIndex) * 0.2}s` }}
                  >
                    {/* Gradient overlay */}
                    <div className={`absolute inset-0 bg-gradient-to-r ${classItem.gradient} opacity-0 group-hover:opacity-30 transition-opacity duration-500`}></div>
                    
                    {/* Glow effect */}
                    <div className={`absolute -inset-1 bg-gradient-to-r ${classItem.gradient} rounded-2xl blur-xl opacity-0 group-hover:opacity-20 transition-opacity duration-500`}></div>
                    ...
                    <div className="relative">
                      <div className="md:flex">
                        ...
                        <div className="md:w-2/5 relative overflow-hidden">
                          ...
                          />
                          <div className={`absolute inset-0 bg-gradient-to-r ${classItem.gradient} opacity-30 group-hover:opacity-50 transition-opacity duration-500`}></div>
                          
                          {/* Level badge on image */}
                          <div className="absolute top-6 left-6">
                            <span className={`px-4 py-2 bg-black/70 text-${classItem.color} text-sm font-light rounded-full border border-${classItem.color}/50 backdrop-blur-sm`}>
                              {classItem.level}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="md:w-3/5 p-8 md:p-12 space-y-8">
                          <div className="space-y-4">
                            <h3 className={`text-4xl md:text-5xl font-serif font-light text-gray-800 group-hover:text-${classItem.color} transition-colors duration-300`}>
                              {classItem.title}
                            </h3>
                            
                            {/* Decorative line */}
                            <div className={`w-20 h-1 bg-gradient-to-r ${classItem.gradient} group-hover:w-32 transition-all duration-500 rounded-full`}></div>
                          </div>
                          ...
                          <div className="bg-white/60 border border-gray-300/30 rounded-xl p-6 backdrop-blur-sm">
                            ...
                            <div className="text-gray-800 font-light text-lg">{classItem.schedule}</div>
                          </div>
                        </div>
                      </div>
                    </div>
